...
async function main(){
  ...
  */
  ...
  const template = handlebars.compile(fs.readFileSync(path.join(__dirname,'feed.hbs')).toString());
  ...
  function redable(element){
    ...
    // $('*').contents().each(function(i,branch) {
    //   // if( branch.nodeType === 3 ){
    //   //   const text = $.html(branch).trim();
    //   //   plain += text + ' ';
    //   // }
    //   plain += $(branch).text() + " ";
    // });
  ...
  const feed = data.map((item,index)=>{
    ...
    for( let element of item.data ){
      if(!types[element.type]) types[element.type] = new Set();
      Object.keys(element).map(i=>types[element.type].add(i))
  ...
  fs.writeFileSync( path.resolve(options.data.feed), JSON.stringify(feed, null, '  ') );
